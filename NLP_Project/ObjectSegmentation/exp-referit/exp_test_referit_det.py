...
pretrained_model = './exp-referit/tfmodel/referit_fc8_detect_iteration_25000.tfmodel'
...
# Local image feature
fc8_crop = vgg_net.vgg_fc8(imagecrop_batch, 'vgg_local', apply_dropout=False)
...
# Pre-allocate arrays
imagecrop_val = np.zeros((N, 224, 224, 3), dtype=np.float32)
...
for n_im in range(num_im):
    ...
    imagecrop_val[:num_proposal, ...] = image_processing.crop_bboxes_subtract_mean(
        im, bbox_proposals, 224, vgg_net.channel_mean)
    fc8_crop_val = sess.run(fc8_crop, feed_dict={imagecrop_batch:imagecrop_val})
    ...
    for imagecrop_name, gt_bbox, description in flat_query_dict[imname]:
