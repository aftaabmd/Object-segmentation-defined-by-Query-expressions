...
imagecrop_list = query_dict.keys()
vocab_dict = text_processing.load_vocab_dict_from_file(vocab_file)

# Collect training samples
training_samples = []
num_imagecrop = len(imagecrop_list)
for n_imagecrop in range(num_imagecrop):
    if n_imagecrop % 200 == 0: print('processing %d / %d' % (n_imagecrop+1, num_imagecrop))
    imagecrop_name = imagecrop_list[n_imagecrop]

    # Image and mask
    imagename = imagecrop_name.split('_', 1)[0] + '.jpg'
    mask_name = imagecrop_name + '.mat'
    for description in query_dict[imagecrop_name]:
        training_samples.append((imagename, mask_name, description))
...
imagecrop_batch = np.zeros((N, input_H, input_W, 3), dtype=np.uint8)
...
for n_batch in range(num_batch):
    ...
    for n_sample in range(batch_begin, batch_end):
        imagename, mask_name, description = shuffled_training_samples[n_sample]
        im = skimage.io.imread(image_dir + imagename)
        ...
        processed_mask = image_processing.resize_and_pad(mask, input_H, input_W)
        ...
        imagecrop_batch[n_sample-batch_begin, ...] = processed_im
